...
paneA2 <- ggplot() +
  geom_boxplot(aes(x = trt_label, y = fob_77, fill = trt_type), outlier.shape = NA, alpha = 0.6, color = "black", data = temp1) +
    scale_colour_manual(
...
paneB2 <- ggplot() +
  geom_boxplot(aes(x = trt_label, y = post_kgs_C, fill = trt_type), outlier.shape = NA, alpha = 0.6, color = "black", data = temp2) +
  geom_point(aes(x = trt_label, y = post_kgs_C), shape=20, size=10, color="black", fill="black", data = emplot_weight) +
    scale_colour_manual(
    limits = c("Control", "Synthetic", "Natural"),
    labels = c("Control", "Synthetic", "Natural"),
...
ggsave("outputs/strength_weight v3.png", width = 17.25, height = 8, units = "in")
```
